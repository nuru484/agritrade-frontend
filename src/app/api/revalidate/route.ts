...
import { ALL_CACHE_TAGS } from "@/config/cache-tags";
...
/**
 * On-demand cache invalidation, called by agritrade-backend after a
 * successful admin write so changes go live immediately instead of waiting
 * out the scheduled ISR window (see src/config/cache-tags.ts for the
 * contract).
 *
 * Tags the caller sends that this app doesn't cache are reported as `skipped`,
 * not errors - the backend broadcasts one tag list and each frontend purges
 * only what it recognises. Purging never makes a page dynamic; each affected
 * page rebuilds once on its next request.
 */
export async function POST(request: Request) {
  ...
  for (const tag of new Set(tags)) {
    if (ALL_CACHE_TAGS.has(tag)) {
      // 'max' = purge immediately; the next request blocks for fresh data
      // instead of serving stale while revalidating in the background.
