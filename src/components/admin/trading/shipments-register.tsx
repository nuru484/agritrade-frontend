  ...
  return (
    <div>
      <div className="mb-3.5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-[22px] font-bold tracking-[-0.01em] text-ink">Shipments</h1>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {shipmentRows.map((sh) => (
          ...
          >
            ...
            <div className="text-[14.5px] font-semibold text-ink">{sh.route}</div>
            <div className="mt-[3px] text-[12.5px] text-soil">{sh.cargo}</div>
            <div className="mt-0.5 text-[12px] text-soil/70">{sh.meta}</div>
          </Link>
        ))}
      </div>
    </div>
