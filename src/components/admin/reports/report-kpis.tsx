...
export function ReportKpis() {
  return (
    <div className="mb-5 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
      {REPORT_KPIS.map((k) => (
        <AdminCard key={k.label} className="rounded-lg px-[15px] py-[13px]">
          <div className="mb-[7px] text-[10.5px] font-semibold uppercase tracking-[0.1em] text-slate-500">
            {k.label}
          </div>
          <Mono
            className={cn(
              "block whitespace-nowrap text-[19px] font-bold tracking-[-0.02em]",
              k.profit ? "text-[#2F5E3D]" : "text-slate-900",
            )}
          >
            {k.value}
          </Mono>
          <div
            className={cn(
              ...
              k.deltaUp ? "text-[#2F5E3D]" : "text-slate-500",
            )}
          >
            {k.delta}
          </div>
