    ...
    <AdminCard className="overflow-hidden">
      ...
      {PL_ROWS.map((row) => (
        <div
          key={row.label}
          className={cn(
            "flex items-center justify-between gap-3 border-b border-soil/15 py-2.5 pr-5 text-[13px] last:border-b-0",
            row.indent ? "pl-[34px]" : "pl-5",
            row.emphasis === "subtotal" && "bg-surface-alt/60",
        ...
        >
          <span
            className={cn(
              ...
              row.indent && "font-normal text-soil",
              !row.indent && !row.emphasis && "font-semibold text-ink",
              row.emphasis === "subtotal" && "font-bold text-ink",
